...
const DiscussionPanel = () => {
  ...
  const togglePin = async (id: string, current: boolean) => {
    await supabase.from("discussions" as any).update({ pinned: !current } as any).eq("id", id);
    toast.success(current ? "Unpinned." : "Pinned.");
    fetchDiscussions();
  };

  const toggleLock = async (id: string, current: boolean) => {
    await supabase.from("discussions" as any).update({ locked: !current } as any).eq("id", id);
    toast.success(current ? "Unlocked." : "Locked.");
    fetchDiscussions();
    if (selectedDiscussion?.id === id) {
      setSelectedDiscussion({ ...selectedDiscussion, locked: !current });
    }
  };

  const deleteDiscussion = async (id: string) => {
    await supabase.from("discussions" as any).delete().eq("id", id);
  ...
  if (selectedDiscussion) {
    return (
      <div className="space-y-4">
        <Button variant="ghost" size="sm" onClick={() => setSelectedDiscussion(null)} className="text-muted-foreground">
          ← Back to discussions
        </Button>
          ...
          <CardContent className="space-y-4">
            <div className="p-4 rounded-lg bg-secondary/30 border border-border">
              ...
              <p className="text-xs text-muted-foreground mt-2">
                {format(new Date(selectedDiscussion.created_at), "MMM d, yyyy HH:mm")}
              </p>
            </div>
            ...
            {!selectedDiscussion.locked && (
              <div className="flex gap-2">
                ...
                />
                <Button onClick={handleReply} size="icon" className="bg-gold text-gold-foreground">
                  <Send className="h-4 w-4" />
                </Button>
              </div>
            )}
            {selectedDiscussion.locked && (
              <p className="text-center text-sm text-muted-foreground py-2">This discussion is locked.</p>
            )}
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-card border-border">
        <CardHeader>
          <CardTitle className="font-display text-lg flex items-center gap-2">
            <Plus className="h-5 w-5 text-gold" /> New Discussion
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label>Title</Label>
            <Input value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="Discussion title..." className="bg-secondary border-border" />
          </div>
          <div className="space-y-2">
            <Label>Body</Label>
            <Textarea value={newBody} onChange={e => setNewBody(e.target.value)} placeholder="Write your discussion post..." className="bg-secondary border-border min-h-[100px]" />
          </div>
          <Button onClick={handleCreate} disabled={creating || !newTitle.trim() || !newBody.trim()} className="bg-gold text-gold-foreground hover:bg-gold/90">
      ...
      <Card className="bg-card border-border">
        <CardHeader>
          <CardTitle className="font-display text-lg flex items-center gap-2">
            <MessageSquare className="h-5 w-5 text-gold" /> Discussions
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
            {discussions.map(d => (
              <div
                key={d.id}
                className="flex items-center justify-between p-3 rounded-lg bg-secondary/30 border border-border hover:border-gold/20 transition-colors cursor-pointer"
